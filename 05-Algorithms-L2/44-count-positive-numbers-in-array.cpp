...
int countPositives(int arr[100], int size) {
	int pos = 0;
	for (int i = 0; i < size; i++) {
		if ((arr[i] > 0)) {
			pos++;
...
void printPositivesCountOf(int arr[100], int size) {
	int pos = countPositives(arr, size);
	std::cout << "\nPositives count is " << pos;
}
...
int main() {
	...
	printPositivesCountOf(array, size);
	return 0;
}